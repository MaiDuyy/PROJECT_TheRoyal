        ...
        if(phong.getTrangThai().equals("Đã đặt")) {
        	...
        	nhanPhong.addMouseListener(new MouseAdapter() {
            	...
            	public void mouseClicked(MouseEvent e) {
					...
					Phong thayDoiTrangThai = RMIClient.getInstance().getPhongService().getPhongByMaPhong(phong.getMaPhong());
					thayDoiTrangThai.setTrangThai("Đang ở");
                    RMIClient.getInstance().getPhongService().updateTinhTrang(String.valueOf(thayDoiTrangThai), "Đang ở");
...
//            		loadCTHoaDonToTable(phong);
        ...
        lblNewLabel_4.addMouseListener(new MouseAdapter() {
        	...
        	public void mouseClicked(MouseEvent e) {
        		...
        		KhachHangService khTimDuoc = RMIClient.getInstance().getKhachHangService();
        ...
        lblNewLabel_5.setIcon(new ImageIcon("icon/khachhang_bar.png"));
        ...
        ngayNhanPhong.getCalendarButton().setIcon(new ImageIcon("icon/date.png"));
        ...
        ngayTraPhong.getCalendarButton().setIcon(new ImageIcon("icon/date.png"));
     ...
     // Thêm sự kiện cho bảng Sản phẩm
        ...
        timKiemSP.addMouseListener(new MouseAdapter() {
            ...
            @Override
        	public void mouseClicked(MouseEvent e) {
        		SanPhamService spDao = RMIClient.getInstance().getSanPhamService();
        		SanPham sp = new SanPham();
        		sp = spDao.timSanPhamTheoMaHoacTheoTen(txtTmKimSP.getText());
        		  if (sp != null) {
        ...
        timKiemDV.addMouseListener(new MouseAdapter() {
            ...
            @Override
        	public void mouseClicked(MouseEvent e) {
        		DichVuService dvDao =  RMIClient.getInstance().getDichVuService();
        ...
        timKiemDV.setIcon(new ImageIcon("icon/search.png"));
...
//
        ...
        lblNewLabel_1.setIcon(new ImageIcon("icon/exit.png"));
    ...
    public void loadSanPhamToTable() {
        SanPhamService sanPhamDao = RMIClient.getInstance().getSanPhamService();
        ArrayList<SanPham> listSanPham = (ArrayList<SanPham>) sanPhamDao.getAll();
    ...
    public void loadDichVuToTable() {
        DichVuService dichVuDao = RMIClient.getInstance().getDichVuService();
        ArrayList<DichVu> listDichVu = (ArrayList<DichVu>) dichVuDao.getAll();
    ...
    public void loadDonDatPhong(String maPhong) {
    	...
    	DonDatPhongService ddpDao = RMIClient.getInstance().getDonDatPhongService();
    	ddp = ddpDao.getDonDatPhongByRoomId(maPhong);

    	KhachHang kh = new KhachHang();
    	KhachHangService khDao= RMIClient.getInstance().getKhachHangService();
		...
		PhongService phongDao = RMIClient.getInstance().getPhongService();
...
//	     	};
    ...
    public void luuCTHoaDonTuBangSPDV(Phong phong) {
        HoaDonService hdDAO = RMIClient.getInstance().getHoaDonService();
        ...
        for (int i = 0; i < modelSPDV.getRowCount(); i++) {
            ...
            SanPhamService sanPhamDao = RMIClient.getInstance().getSanPhamService();
            SanPham sanPham = sanPhamDao.timSanPhamTheoMaHoacTheoTen(tenSPDV);
            ...
            // Lưu vào CTHoaDon
            CTHoaDonService ctHoaDonDao = RMIClient.getInstance().getCtHoaDonService();
            ctHoaDonDao.insert(ctHoaDon);
        }
    }
    ...
    public void loadCTHoaDonToTable(Phong phong) {
        ...
        CTHoaDonService ctHoaDonDao = RMIClient.getInstance().getCtHoaDonService();
        List<CTHoaDon> listCTHD = ctHoaDonDao.getListCTHoaDonByMaHD(hoaDon.getMaHD());
