...
public class ThemSanPham_Dialog extends JDialog {
    ...
    public ThemSanPham_Dialog(javax.swing.JInternalFrame parent, javax.swing.JFrame owner, boolean modal) {
        super(owner, modal);
        GUI();
...
//                 	  System.exit(0);
    ...
    private void themNV() {
        ...
        if (validData(ADD)) {
            try {
                boolean result = SanPhamDAO.getInstance().insert(sp);
                String maKH = SanPhamDAO.getInstance().getLatestID();
                txtMa.setText(String.valueOf(maKH));
                if (result == true) {
                    double number = sp.getGiaSP();
                    DecimalFormat df = new DecimalFormat("#,###.##");
                    String donGia = df.format(number);
                    home.tableModelSanPham.addRow(new Object[] {
                        maKH,
                        sp.getTenSP(),
                        sp.getMoTa(),
                        donGia,
                    ...
                    dispose();
                    home.tableModelSanPham.fireTableDataChanged();
                    home.loadListDichVu();
                } else {
