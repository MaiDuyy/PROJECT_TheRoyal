...
public class SuaPhong_Dialog extends JDialog {
    ...
    public SuaPhong_Dialog(javax.swing.JInternalFrame parent, javax.swing.JFrame owner, boolean modal) {
		...
		txtTenP.setText(p.getTenPhong());
		cboMaLoaiP.setSelectedItem(p.getLoaiPhong().getMaLoai());
		txtgiaTien.setText(String.valueOf(p.getGiaTien()).replace(".0", ""));
		spinSoGiuong.setValue(p.getSoGiuong());
		spinSoTrem.setValue(p.getSoTreEm());
		spinSoNguoiLon.setValue(p.getSoNguoiLon());
		cboTrangThai.setSelectedItem(p.getTrangThai());
        loadCboMaLoaiPhong();
    ...
    public void GUI() {
        setSize(614, 576);
        setTitle("Sửa phòng");
        HeaderTitle title = new HeaderTitle("SỬA PHÒNG");
