...
public class QLUuDai_GUI extends JInternalFrame implements ActionListener {
   ...
   public DefaultTableModel tableModelUD;
	...
	private ThemKhuyenMai_Dialog themkhuyenmaidialog;
...
//	        searchPanel.add(btnTim);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource().equals(btnThem)) {
            ...
            try {
                KhuyenMai sp = getSanPhanSelect();
                String tenKH = sp.getTenKM();
                ...
                if (confirm == JOptionPane.YES_OPTION) {
                    KhuyenMaiDAO.getInstance().delete(sp);
	...
	private void DocDuLieuNVVaoTable() {
        ...
        for ( KhuyenMai item: dsKM) {
            String ngayBatDau = formatDate(item.getNgayBatDau());
            String ngayKetThuc = formatDate(item.getNgayKetThuc());
            float giaTri = item.getGiaTriKM();
...
//        Date ngayKT = new Date(ngayketthuc.getDate().getTime());
    ...
    public void showMessage(String message, JTextField txt) {
     ...
     public KhuyenMai getSanPhanSelect() {
         int i_row = tblUuDai.getSelectedRow();
         if (i_row == -1) {
             throw new IllegalArgumentException("Không chọn được dòng nào");
         }
         String maNV = tblUuDai.getValueAt(i_row, 0).toString();
         return KhuyenMaiDAO.getInstance().getKhuyenMaiTheoMa1(maNV);
     }
}
